...
public class UpcomingFragment extends Fragment implements NetworkResponseListeners, SwipeRefreshLayout.OnRefreshListener {
    ...
    private void refresh() {
        if (NetworkProvider.isConnected(getContext())) {
            ...
            moviesCategoryViewmodel.getMovies("upcoming", 1);
        } else {
    ...
    private void pagination() {
        server.getPaginationMovies(PaginationClient.getClient("upcoming", API_KEY, pageNumber)).enqueue(new Callback<Responses>() {
            @Override
