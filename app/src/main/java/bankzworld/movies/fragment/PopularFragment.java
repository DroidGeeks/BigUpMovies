 ...
 */
public class PopularFragment extends Fragment implements NetworkResponseListeners, SwipeRefreshLayout.OnRefreshListener {
    ...
    private void refresh() {
        if (NetworkProvider.isConnected(getContext())) {
            ...
            moviesCategoryViewmodel.getMovies("popular", 1);
        } else {
    ...
    private void pagination() {
        server.getPaginationMovies(PaginationClient.getClient("popular", API_KEY, pageNumber)).enqueue(new Callback<Responses>() {
            @Override
